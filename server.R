function(input, output, session) {
  ...
  observe({
    opts = if (length(input$fuel_types) > 1) {
      resp_opts
    } else {
      resp_opts[resp_opts != "percent_selected"]
    }
    if (rv$resp_last != input$resp) {
      rv$resp_last = input$resp
    }
  ...
  barSumm <- reactive({
    czYearFuelSub() |>
      ...
      mutate(
        ...
        tooltip_text = make_plot_tooltip(year, fuel_type, count, percent)
      )
  })
  ...
  observeEvent(input$map_filter, {
    opts = if (input$map_filter == "county") {
      resp_map_opts
    } else {
      resp_map_opts[resp_map_opts != "per_capita"]
    }
    if (rv$resp_map_last != input$resp_map) {
      rv$resp_map_last = input$resp_map
    }
  ...
  spatialPopSumm <- reactive({
    ...
    tmp = spatialSub() |>
      ...
      filter(count > 0) |>
      mutate(
        per_sqmi = count / area_sqmi,
        popup = paste0(
          "<strong>",
          top_row(input$map_filter, .data[[input$map_filter]]),
          " (",
          year,
          ")</strong><br>",
    ...
    if (input$map_filter == "county") {
      popest_sub = county_popest[
        county_popest[["county"]] %in% input$counties,
      ]
      tmp = tmp |>
        left_join(popest_sub, by = join_by(county, year)) |>
        mutate(
          per_capita = count / popest,
  ...
  legendTitle <- reactive({
    if (input$resp_map == "count") {
      "Number of<br>Vehicles"
    } else if (input$resp_map == "per_capita") {
      "Vehicles Per<br>Capita"
    } else {
      "Vehicles Per<br>Square Mile"
    }
  })
  ...
  observe({
    ...
    proxy |>
      addPolygons(
        ...
        color = "black",
        fillOpacity = 0.7,
        fillColor = colorNumeric("Blues", dfx[[input$resp_map]])(dfx[[
          input$resp_map
        ]]),
      ...
      addLegend(
        "topright",
        pal = colorNumeric("Blues", dfx[[input$resp_map]]),
        values = dfx[[input$resp_map]],
        title = legendTitle(),
        opacity = 1
      )
  })
