...
county_popest = prep_census(
  "/2010-2019/counties/totals/co-est2019-alldata.csv"
) |>
  bind_rows(
    prep_census("/2020-2025/counties/totals/co-est2025-alldata.csv")
  )
...
if (!file.exists(ld_file)) {
  download.file(
    "https://www.energy.ca.gov/filebrowser/download/9606?fid=9606",
    ld_file,
    mode = "wb"
  )
}
...
zip_pop = readxl::read_xlsx(
  file.path("data", "Vehicle_Population.xlsx"),
  sheet = "ZIP"
) |>
...
cities = read_sf(
  "https://egis.fire.ca.gov/arcgis/rest/services/FRAP/Incorp/FeatureServer/0/query?outFields=*&where=1%3D1&f=geojson"
) |>
  st_make_valid()
...
counties = read_sf(
  "https://services3.arcgis.com/uknczv4rpevve42E/arcgis/rest/services/California_County_Boundaries_and_Identifiers_Blue_Version_view/FeatureServer/1/query?outFields=*&where=1%3D1&f=geojson"
) |>
  st_make_valid()
...
# resorted to downloading from the link above and placing in data folder
zip_codes = read_sf(
  file.path(
    "data",
    "USA_ZIP_Code_Areas_anaylsis_-8030337519481127108.geojson"
  )
) |>
  st_make_valid()
...
intersects <- function(x, y, x_col, y_col) {
  ...
  out = list()
  cli::cli_progress_bar("Finding intersections...", total = nrow(x))
  for (i in 1:nrow(x)) {
